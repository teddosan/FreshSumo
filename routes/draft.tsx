...
export const handler: Handlers<DraftData> = {
  GET(_req: Request, ctx) {
    ...
    const rows = db.query("SELECT name, owner FROM wrestlers ORDER BY owner ASC");
...
export default function DraftPage({ data }: PageProps<DraftData>) {
  return (
    <div class="min-h-screen bg-slate-50 p-4 md:p-8">
      <div class="max-w-xl mx-auto">
        <header class="mb-8">
          <a href="/" class="text-indigo-600 font-bold text-sm">← Back to Dashboard</a>
        ...
        <div class="space-y-6">
          <section>
            <h3 class="font-bold text-slate-700 mb-3 px-1">Your Current Stable</h3>
            <div class="space-y-2">
              {data.myWrestlers.length > 0 ? (
                data.myWrestlers.map((w) => (
                  <div class="flex justify-between items-center p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                    <div>
                      <span class="font-bold text-indigo-900 block">{w.name}</span>
                      <span class="text-[10px] uppercase font-bold text-slate-400">Owner: {w.owner}</span>
                    </div>
          ...
          <div class="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
            ...
            <p class="text-sm text-indigo-700 mb-4">Search the banzuke to add to your team.</p>
            <button disabled class="w-full py-3 bg-indigo-200 text-indigo-400 font-bold rounded-lg cursor-not-allowed">
              Search Coming Soon...
            </button>
          </div>
        </div>
